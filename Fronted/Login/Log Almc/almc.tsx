...
const Almc = ({ onForgotPassword, onBack, onLoginSuccess }: AlmcProps) => {
  
  const handleLogin = (username: string, password: string) => {
    console.log('ALMC login:', username, password);
    
    // Aquí puedes agregar tu lógica de autenticación para almacenero
    // Por ahora, validación simple de ejemplo
    if (username && password) {
      // Simular validación exitosa
      console.log('Login exitoso para almacenero');
      if (onLoginSuccess) {
        onLoginSuccess();
      }
    } else {
  ...
  const handleHelpClick = () => {
    console.log('Help clicked for ALMC');
    ...
    <div className="flex h-screen w-full overflow-hidden">
      ...
      <div className="w-8/12 -ml-24 bg-white flex items-center justify-center px-20 py-24 min-h-[90vh] rounded-l-[60px] rounded-r-none shadow-lg relative z-10">
        ...
        <LoginForm 
          ...
          helpLinkText="¿Asistencia técnica?"
